...
static ssize_t my_read(struct file* f, char *buf, size_t count, loff_t *f_pos)
{
	...
	if (ret >= 0)
		ret = raw_copy_to_user(buf, tmp, count) ? -EFAULT : count;
	kfree(tmp);
	return ret;
}
...
static int dummy_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	init_result = alloc_chrdev_region(&data->devt, 0, 1, "i2c_ply");
	data->client = client;
	...
	if ((data->class = class_create(THIS_MODULE, "i2cplay")) == NULL)
	{
		printk( KERN_ALERT "Class creation failed\n" );
		unregister_chrdev_region(data->devt, 1);
		return -1;
	}
	if(device_create(data->class, NULL, data->devt, NULL, "i2c_ply%d", 0) == NULL)
	{
